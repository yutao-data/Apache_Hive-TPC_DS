...
query_directory = '/opt/hive/tpcds-kit/hive_queries/netezza_queries/opt_queries'
results_directory = '/scripts/results/results_1gb_optimzed_1times'
...
with open(f"{results_directory}/query_execution_time.csv", "w", newline='') as exec_time_file:
    ...
    for query_file in listdir(query_directory):
        if query_file.endswith(".sql"):
            ...
            match = re.match(r'q(\d+)_opt\.sql', query_file) 
            if match:
            ...
            for run_number in range(1,2):
                ...
                beeline_cmd = f'beeline -u jdbc:hive2://localhost:10000/1gb -f {query_path} > {output_path}'
